function xn = P2_zad2b_values_x(x0, x, y, t)
...
for n = 2:length(xn)
    ...
    r = roots([(1/2)*rxx*(t(n)-t(n-1)),...
        -1 + (1/2)*rx*(t(n)-t(n-1)) + (1/2)*rxy*y(n)*(t(n)-t(n-1)),...
        xn(n-1)*(1 + (1/2)*rx*(t(n)-t(n-1)) +...
        (1/2)*rxx*xn(n-1)*(t(n)-t(n-1)) + (1/2)*rxy*y(n-1)*(t(n)-t(n-1)))]);
    xn(n) = max(r);
end